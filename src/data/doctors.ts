import { Doctor } from "../types";

export const doctors: Doctor[] = [
  {
    id: "1",
    name: "ডাঃ মোঃ আবু সাঈদ",
    specialty: "Medicine",
    degree: "MBBS, BCS (Health), FCPS (Medicine)",
    hospital: "ব্রাহ্মণবাড়িয়া জেনারেল হাসপাতাল",
    chamber: "ল্যাব এইড ডায়াগনস্টিক, ব্রাহ্মণবাড়িয়া",
    phone: "01711-XXXXXX",
    visitingHours: "বিকাল ৪টা - রাত ৮টা",
    bookingFee: 800,
    experience: "১৫ বছরের বেশি অভিজ্ঞতা",
    about: "ডাঃ মোঃ আবু সাঈদ একজন মেডিসিন বিশেষজ্ঞ। তিনি দীর্ঘ ১৫ বছর ধরে ব্রাহ্মণবাড়িয়া জেনারেল হাসপাতালে কর্মরত আছেন এবং রোগীদের বিশ্বস্ততার সাথে চিকিৎসা সেবা প্রদান করছেন।"
  },
  {
    id: "2",
    name: "ডাঃ ফৌজিয়া আক্তার",
    specialty: "Gynecology",
    degree: "MBBS, DGO, MCPS (Gynae)",
    hospital: "ব্রাহ্মণবাড়িয়া মেডিকেল কলেজ",
    chamber: "পপুলার ডায়াগনস্টিক সেন্টার, ব্রাহ্মণবাড়িয়া",
    phone: "01819-XXXXXX",
    visitingHours: "বিকাল ৩টা - সন্ধ্যা ৭টা",
    bookingFee: 700,
    experience: "১০ বছরের অভিজ্ঞতা",
    about: "ডাঃ ফৌজিয়া আক্তার একজন অভিজ্ঞ স্ত্রী ও প্রসূতি রোগ বিশেষজ্ঞ। তিনি ব্রাহ্মণবাড়িয়া মেডিকেল কলেজে কর্মরত আছেন।"
  },
  {
    id: "3",
    name: "ডাঃ এস. এম. শাহিন",
    specialty: "Cardiology",
    degree: "MBBS, MD (Cardiology)",
    hospital: "জাতীয় হৃদরোগ ইনস্টিটিউট",
    chamber: "মেডিনোভা মেডিকেল সার্ভিসেস, ব্রাহ্মণবাড়িয়া",
    phone: "01911-XXXXXX",
    visitingHours: "শুক্রবার ও শনিবার",
    bookingFee: 1000,
    experience: "১২ বছরের অভিজ্ঞতা",
    about: "ডাঃ এস. এম. শাহিন একজন স্বনামধন্য হৃদরোগ বিশেষজ্ঞ। তিনি জাতীয় হৃদরোগ ইনস্টিটিউটে কর্মরত এবং ব্রাহ্মণবাড়িয়ায় নিয়মিত রোগী দেখেন।"
  },
  {
    id: "4",
    name: "ডাঃ মোঃ কামরুল হাসান",
    specialty: "Pediatrics",
    degree: "MBBS, DCH (Child)",
    hospital: "ব্রাহ্মণবাড়িয়া জেনারেল হাসপাতাল",
    chamber: "ব্রাহ্মণবাড়িয়া শিশু হাসপাতাল",
    phone: "01712-XXXXXX",
    visitingHours: "বিকাল ৫টা - রাত ৯টা",
    bookingFee: 600,
    experience: "৮ বছরের অভিজ্ঞতা",
    about: "ডাঃ মোঃ কামরুল হাসান একজন শিশু রোগ বিশেষজ্ঞ। তিনি শিশুদের বিভিন্ন জটিল রোগের চিকিৎসায় অত্যন্ত পারদর্শী।"
  },
  {
    id: "2",
    name: "ডাঃ ফৌজিয়া আক্তার",
    specialty: "Gynecology",
    degree: "MBBS, DGO, MCPS (Gynae)",
    hospital: "ব্রাহ্মণবাড়িয়া মেডিকেল কলেজ",
    chamber: "পপুলার ডায়াগনস্টিক সেন্টার, ব্রাহ্মণবাড়িয়া",
    phone: "01819-XXXXXX",
    visitingHours: "বিকাল ৩টা - সন্ধ্যা ৭টা",
    bookingFee: 700,
    experience: "১০ বছরের অভিজ্ঞতা",
    about: "ডাঃ ফৌজিয়া আক্তার একজন অভিজ্ঞ স্ত্রী ও প্রসূতি রোগ বিশেষজ্ঞ। তিনি ব্রাহ্মণবাড়িয়া মেডিকেল কলেজে কর্মরত আছেন।"
  },
  {
    id: "5",
    name: "ডাঃ তানিয়া সুলতানা",
    specialty: "Dental",
    degree: "BDS, PGT (Dental)",
    hospital: "প্রাইভেট প্র্যাকটিস",
    chamber: "স্মাইল ডেন্টাল কেয়ার, টিএ রোড",
    phone: "01675-XXXXXX",
    visitingHours: "সকাল ১০টা - রাত ৮টা",
    bookingFee: 500,
    experience: "৫ বছরের অভিজ্ঞতা",
    about: "ডাঃ তানিয়া সুলতানা একজন ডেন্টাল সার্জন। তিনি দাঁত ও মুখের বিভিন্ন রোগের আধুনিক চিকিৎসায় অভিজ্ঞ।"
  }
];
